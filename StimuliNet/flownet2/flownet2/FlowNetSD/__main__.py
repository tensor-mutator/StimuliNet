...
def main(args: Namespace) -> None:
    if args.train:
       ...
       pipeline = Pipeline(FlowNetSD, "LONG_SCHEDULE", resolution, resolution, checkpoint_path=weights_path, config=config.LOSS_EVENT+config.SAVE_FLOW)
    ...
    if args.get_graph:
       pipeline = Pipeline(FlowNetSD, "LONG_SCHEDULE", (512, 512), (512, 512), checkpoint_path=weights_path, config=config.LOSS_EVENT+config.SAVE_FLOW)
